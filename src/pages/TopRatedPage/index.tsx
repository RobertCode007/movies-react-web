...
const TopRatedPage: React.FC = () => {
  ...
  const [viewMode, setViewMode] = useState<ViewMode>(() => {
    const saved = localStorage.getItem('topRatedMovieViewMode') as ViewMode | null;
    return saved || 'card';
  });
  ...
  const requestFn = useCallback((pageNum: number) => {
    ...
    return getTopRatedMovies({ page: pageNum, language });
  }, [currentLanguage]);
  ...
  const handleViewModeToggle = () => {
    ...
    setViewMode(newMode);
    localStorage.setItem('topRatedMovieViewMode', newMode);
  };
    ...
    <div className={styles.page}>
      <div className={styles.container}>
        <div className={styles.page_header}>
          <h2 className={styles.page_title}>{t('navigation.topRated')}</h2>
          <button
